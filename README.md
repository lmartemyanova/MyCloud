...
Развернутое приложение: [mycloudstorage](https://mycloudstorage.ru/).
...
```bash
sudo nano /etc/nginx/sites-available/mycloud  # или ваш конфиг
```
...
> ## ⚠ Публичные ссылки
> Важно: на данном этапе функция создания публичной ссылки не работает, так как она требует корректной генерации абсолютных URL, доступных извне.
> Это возможно только при подключении доменного имени и включении HTTPS.
> 
> Если вам необходимо использовать публичные ссылки для обмена файлами — перейдите к следующему разделу и подключите домен.

# 🌐 Подключение домена и HTTPS

## 📌 Предварительные требования

1. У вас должен быть зарегистрирован домен (например, через reg.ru).
2. DNS-записи домена должны указывать на ваш сервер:
   - Тип A → ваш публичный IP-адрес
...
|----------|----------|-----------------|
| A        | @        | 135.125.xxx.xxx |
| A        | www      | 135.125.xxx.xxx |

> ⚠️ DNS изменения могут занять до 24 часов, но чаще ~10–30 минут.

## 1. Получение HTTPS-сертификата с помощью Certbot
...
```bash
sudo certbot --nginx -d mycloudstorage.ru -d www.mycloudstorage.ru 
# далее на примере домена mycloudstorage.ru и его www-поддомена
```
...
- Укажите email.
- Разрешите редирект (выбор 2 — “Redirect HTTP to HTTPS”).
...
## 2. Правка Nginx-конфига

Certbot автоматически добавит нужные директивы в ```/etc/nginx/sites-available/mycloud```.
...
Вручную нужно только изменить строку server_name:

```nginx
server {
    server_name mycloudstorage.ru www.mycloudstorage.ru; # your-domain.ru www.your-domain.ru
    ...
}
```

Также для снятия ограничения в 1 Мб для загружаемых файлов в конфиг nginx следует добавить:
...
Открываем .env в backend:

```bash
nano /home/your_username/MyCloud/backend/.env
```
...
```env
DEBUG=False
ALLOWED_HOSTS=mycloudstorage.ru,www.mycloudstorage.ru,127.0.0.1,localhost
```

> Важно: ALLOWED_HOSTS не должен быть строкой-псевдосписком (в квадратных скобках), только через запятую.

## 4. Обновление CORS (если используется)

В backend/settings.py:

```python
CSRF_TRUSTED_ORIGINS = [
    "https://mycloudstorage.ru",
    "https://www.mycloudstorage.ru"
]

CORS_ALLOWED_ORIGINS = [
    "http://localhost:5173",  # адрес фронта для локальной разработки, можно оставить
    "https://mycloudstorage.ru",
    "https://www.mycloudstorage.ru",
]
```

## 5. Обновление фронта (если есть .env)

Если используется .env во фронтенде:

```bash
nano /home/your_username/MyCloud/frontend/.env
```
...
```env
VITE_API_URL=/api
VITE_FRONTEND_URL=https://mycloudstorage.ru
```
...
Теперь проект доступен по https://mycloudstorage.ru (https://your-domain.ru).
Публичные ссылки будут формироваться корректно с учетом доменного имени.
